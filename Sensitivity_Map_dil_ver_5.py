...
\'save sens map\' option even while the images are still beign processed.\n\
 if you want to halt the processing, just close and relaunch the app.')
    ...
    def initalize(self):
        ...
        for filename in os.listdir(filepat):
            ...
            if img is not None:
                if j < 10:
                    ...
                    initialize_data = rgb2gray((img), img.shape)
                    threshM += np.nanmean(initialize_data)
                    threshSTD += np.nanstd(initialize_data) ** 2
                    print(threshSTD, threshM)

                    Total_Grey_value = np.zeros_like(initialize_data)
                    Times_raw_imgked = np.zeros_like(initialize_data)
                    j += 1
        ...
        h = GT
        return GT,BT,h,filepat,numberofimages,Times_raw_imgked,Total_Grey_value

                   
          
    def Sensitivity_Map(self):
        tic = time.perf_counter()
        GT,BT,h,filepat,numberofimages,Times_raw_imgked,Total_Grey_value = self.initalize()
        self.pb['value'] = 0
        index = 0
